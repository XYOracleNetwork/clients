...
describe('flatMapBoundWitness', () => {
  const payload1 = new PayloadBuilder({ schema: 'network.xyo.debug' }).fields({ nonce: '1' }).build()
  const payload2 = new PayloadBuilder({ schema: 'network.xyo.debug' }).fields({ nonce: '2' }).build()
  describe('BoundWitness with Payloads & nested BoundWitnesses', () => {
    ...
    beforeAll(async () => {
      inner = await new BoundWitnessBuilder(config).payload(await payload2).build()
      outer = await new BoundWitnessBuilder(config).payloads([await payload1, inner[0]]).build()
    ...
    it('extracts the Payloads', async () => {
      expect(result[1]).toBeArray()
      expect(result[1].length).toBe(2)
      expect(result[1]).toContainValues([await payload1, await payload2])
    })
  })
  describe('BoundWitness with Payloads', () => {
    ...
    let result: BoundWitnessMapResult
    beforeAll(async () => {
      outer = await new BoundWitnessBuilder(config).payloads([await payload1, await payload2]).build()
      result = flatMapBoundWitness(outer[0])
    })
    it('extracts the BoundWitnesses', () => {
      expect(result).toBeArray()
