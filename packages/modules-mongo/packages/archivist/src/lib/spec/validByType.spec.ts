...
describe('validByType', () => {
  ...
  describe('BoundWitness with Payloads & nested BoundWitnesses', () => {
    let result: [BoundWitness[], Payload[]]
    beforeAll(async () => {
      const payload1 = await new PayloadBuilder<DebugPayloadWithMongoMeta>({ schema: 'network.xyo.debug' }).fields({ nonce: '1' }).build()
      const payload2 = await new PayloadBuilder<DebugPayloadWithMongoMeta>({ schema: 'network.xyo.debug' }).fields({ nonce: '2' }).build()
      const inner = await (await new BoundWitnessBuilder().signer(await account).payload(payload2)).build()
      const outer = await (await new BoundWitnessBuilder().signer(await account).payloads([payload1, inner[0]])).build()
  ...
  describe('BoundWitness with Payloads', () => {
    ...
    beforeAll(async () => {
      const payload1 = await new PayloadBuilder<DebugPayloadWithMongoMeta>({ schema: 'network.xyo.debug' }).fields({ nonce: '1' }).build()
      const payload2 = await new PayloadBuilder<DebugPayloadWithMongoMeta>({ schema: 'network.xyo.debug' }).fields({ nonce: '2' }).build()
      const outer = await (await new BoundWitnessBuilder().signer(await account).payloads([payload1, payload2])).build()
