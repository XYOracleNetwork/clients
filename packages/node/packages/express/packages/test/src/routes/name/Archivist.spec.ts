...
describe(`/${moduleName}`, () => {
  ...
  beforeAll(async () => {
    ...
    const payloadWrapperA = await PayloadWrapper.wrap(await getNewPayload())
    const payloadWrapperB = await PayloadWrapper.wrap(await getNewPayload())
    const payloadWrapperC = await PayloadWrapper.wrap(await getNewPayload())
