...
export const getSentinelByName = async (name: string, account?: AccountInstance): Promise<SentinelInstance> => {
  const diviner = asSentinelInstance(await getModuleByName(name), () => 'Failed to cast sentinel', { required: true })
...
export const getSentinelByNameFromChildNode = async (name: string, childNodeName: string): Promise<SentinelInstance> => {
  return asSentinelInstance(await getModuleByNameFromChildNode(name, childNodeName), () => 'Failed to cast sentinel', { required: true })
}
