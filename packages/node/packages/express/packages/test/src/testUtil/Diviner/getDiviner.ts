...
export const getDivinerByName = async (name: string, account?: AccountInstance): Promise<DivinerInstance> => {
  const diviner: DivinerInstance = asDivinerInstance(await getModuleByName(name), () => 'Failed to cast diviner', { required: true })
...
export const getDivinerByNameFromChildNode = async (name: string, childNodeName: string): Promise<DivinerInstance> => {
  return asDivinerInstance(await getModuleByNameFromChildNode(name, childNodeName), () => 'Failed to cast diviner', { required: true })
}
